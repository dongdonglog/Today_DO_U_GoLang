...
MCP 是 AI 时代的"USB 接口"，让 AI 模型可以调用外部工具。MCP Server 本质上仍是一个服务端程序：要处理连接、鉴权、工具调用、超时和观测。Go 在这些场景里有成熟的标准库和工程生态。
...
这些需求和传统 API Gateway 很像，Go 的网络编程、并发模型和部署方式都比较适合。
...
Go 的定位很清晰：**在保持较高开发效率的同时，提供足够稳定的服务端性能**。
...
- 会把 AI 能力接入真实业务系统的 Go 工程师，会比只会调用模型 API 更有竞争力
...
本书将带你从基础业务服务开始，逐步走到微服务、云原生和 AI 应用开发。

从下一章开始，我们不再停留在“为什么选 Go”，而是用第一阶段项目 **用户管理服务** 作为主线：先搭建开发环境，再补齐语法基础，然后实现 HTTP API、统一错误处理和可测试的接口设计。
...
- AI 基础设施（MCP Server、AI Gateway）需要稳定的服务端工程能力，Go 是很合适的选择之一
- Go 工程师如果理解 LLM、RAG、Agent 和工具调用，会更容易把 AI 能力接进现有业务系统
