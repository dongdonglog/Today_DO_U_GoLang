...
> 本章基于 **Go 1.25**。（go.mod 声明 go 1.25.0；Go 1.22 的循环变量语义变更为历史背景）语言行为随版本演进，以对应版本官方文档为准。

- `testing` 包文档：https://pkg.go.dev/testing
- Go 博客：Using Subtests and Sub-benchmarks https://go.dev/blog/subtests
- Go 博客：The cover story（测试覆盖率）https://go.dev/blog/cover
- Go 博客：循环变量语义变更（Go 1.22，影响并行子测试）https://go.dev/blog/loopvar-preview
