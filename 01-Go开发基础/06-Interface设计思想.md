...
上一章我们已经为用户管理 API 设计了清晰的错误处理。但还有一个问题：现在的存储实现只是内存版本，后续要换成 MySQL、Redis 缓存，测试时还要用 Mock。

如果业务代码直接依赖 `MemoryStore`，后面替换存储会牵一发动全身。
...
本章从用户管理服务的存储抽象出发，讲透 Go 接口的精髓：接口不是为了“看起来高级”，而是为了让依赖方向更清晰、测试更容易、实现更可替换。
...
1. **灵活性**：调用方可以决定是否需要接口
2. **可扩展性**：可以访问结构体的特定方法
3. **可维护性**：避免把实现细节过早锁死在接口契约里

这不是绝对规则。如果构造函数需要根据配置返回 MySQL、Redis、内存等不同实现，或者你要对外隐藏实现细节，返回接口也是合理选择。关键不是“接口一定不能返回”，而是不要为了看起来抽象而返回接口。
...
// 推荐：先让具体实现跑通，再从调用方真实依赖中抽象接口
type MemoryStore struct {
    // ...
}

// 当服务层只需要 Save / FindByID 时，再定义小接口
type UserStore interface {
    Save(user *User) error
    FindByID(id int) (*User, error)
}
...
接口应该从具体调用场景中抽象出来，而不是提前设计一套“大而全”的能力清单。即使只有一个生产实现，也可能因为测试替身、跨包边界、插件化扩展而定义小接口；判断标准是依赖方向是否更清晰，而不是实现数量是否刚好大于 1。
...
- 小对象且不修改状态，可以用值接收者，让 `T` 和 `*T` 都满足接口
- 需要修改状态、避免复制大对象、结构体包含锁时，使用指针接收者，让 `*T` 满足接口
- 接口没有“接收者”，接收者属于具体类型的方法；真正要判断的是 `T` 还是 `*T` 的方法集满足接口
...
|------|--------|------|
| 类型安全 | 运行时检查 | 编译时检查 |
| 性能 | 可能有装箱、逃逸和类型断言成本 | 通常更容易被编译器优化 |
| 代码简洁 | 需要类型断言 | 直接使用 |
...
- **用泛型**：类型明确，需要类型安全
- **用 interface{}**：确实需要接收任意类型（如 `fmt.Println`）
...
接口调用通常比直接调用多一次间接分发，但在 HTTP、数据库、网络 I/O 这类服务端场景里，瓶颈很少出在这里。是否需要优化，应该以 benchmark 和 pprof 结果为准，而不是看到接口就急着消除。
...
- 用户服务：创建、查询、删除用户
- 存储层：当前使用内存实现，后续可替换为 MySQL 或 Redis
- 可测试：业务层依赖接口，单元测试使用 Mock 存储层

这一步是对第 4、5 章用户管理 API 的重构：HTTP 层不应该知道数据来自内存还是数据库，业务层也不应该被具体存储实现绑死。
...
- 可以访问结构体的特定方法
- 避免过早承诺一套抽象契约
- 但隐藏实现或根据配置返回不同实现时，可以返回接口

**Q5：interface{} 和泛型的区别？**

A：
- `interface{}`：运行时类型检查，可能带来类型断言、逃逸和反射成本
- 泛型：编译时类型检查，通常更容易被编译器优化
- `interface{}`：可以存储任何类型
- 泛型：受类型约束限制
...
- 某些情况下会影响内联
- 会携带具体类型和值
- 多数服务端业务里可以忽略，只有 benchmark 证明它在热点路径上时才需要优化

---

## 6.14 小结

本章从用户管理服务的存储抽象出发，讲透了 Go 接口的精髓：
...
到这里，第一阶段项目已经有了清晰的演进线：

1. 第 3 章用日志分析工具打好语法和测试基础
2. 第 4 章用 `net/http` 做出用户管理 API
3. 第 5 章补齐统一错误处理和 recovery
4. 第 6 章用接口隔离存储层，让服务可测试、可替换
