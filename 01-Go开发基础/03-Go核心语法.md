...
用户管理服务还没上线，Leader 先丢给你一个真实小需求：

> “我们线上日志太多了。先写个命令行工具，统计日志文件里的错误数量，输出 Top 10。后面用户服务出问题时，先靠它快速判断是参数错误、认证错误，还是下游超时。”
...
本章系统讲解 Go 核心语法，但不按语法手册平铺。我们用这个日志分析工具串起变量、函数、结构体、Slice、Map、指针和包组织。下一章写用户管理 HTTP API 时，这些语法会直接用上。
...
```go
// 显式声明类型（浮点字面量默认就是 float64，这里主要为可读性/统一风格）
...
```go
// Go 1.22 之前：三个 goroutine 共享同一个 i，大概率都打印 3
// Go 1.22 之后：每轮迭代 i 是独立变量，分别捕获 0、1、2
// 注意：goroutine 之间没有同步，打印顺序不确定，且 main 可能先退出
for i := 0; i < 3; i++ {
    go func() {
        fmt.Println(i)
    }()
...
![Slice 内存布局](./images/ch03-slice-memory.svg)
...
![Map 底层结构](./images/ch03-map-structure.svg)
...
![方法集规则](./images/ch03-method-set.svg)
...
这个工具不是孤立练习。用户管理服务上线后，常见故障会先体现在日志里：登录失败激增、参数校验错误变多、数据库连接超时、权限校验异常。先写一个可测试的日志分析 CLI，可以帮助你在没有完整监控平台前快速定位问题。
...
func main() {
    ...
    if *filePath == "" {
        fmt.Fprintln(os.Stderr, "用法: log-analyzer -file <日志文件路径> [-top N]")
        os.Exit(1)
    }
...
下一章我们将把这些语法应用到第一个 HTTP 服务里，做出用户管理服务的第一版 API。
...
- Go 博客：Slice 内部实现 https://go.dev/blog/slices-intro
- Go 博客：Map 实现 https://go.dev/blog/maps
- Go 博客：循环变量语义变更（Go 1.22）https://go.dev/blog/loopvar-preview
