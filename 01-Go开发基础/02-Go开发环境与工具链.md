...
你接到第一个任务：把团队的用户管理服务在本地跑起来，并保证所有人提交代码前都能执行同一套检查。

你打开电脑，发现问题比“安装 Go”多得多：

- Go 装哪个版本？团队如何统一？
- 依赖管理用 `go mod`，但私有仓库怎么配？
- 代码格式不统一，同事写的风格各异
- 本地能跑，部署到 Linux 就报错
- 测试、lint、安全扫描、构建命令散落在文档里，没人敢改 CI

这些问题不是技术问题，是**工程问题**。

本章解决一个核心问题：**如何为用户管理服务搭建一套可复制、可检查、可交付的 Go 开发环境**。

本章结束时，你应该能做到：

1. 所有成员使用一致的 Go 版本和模块配置
2. 本地通过 `make test`、`make lint`、`make build` 完成基础检查
3. CI 使用同一套命令，避免“本地能过，流水线不过”
4. 后续章节的 HTTP 服务、错误处理、接口测试都能在这套环境里运行

![环境搭建流程](./images/ch02-dev-env-flow.svg)
...
# 国内网络环境建议配置代理，否则下载依赖可能超时
export GOPROXY=https://goproxy.cn,direct

# 私有仓库（如果需要拉私有 Git 仓库的依赖）
# GOPRIVATE 会同时让这些前缀跳过 GOPROXY 和 GOSUMDB 校验
...
| `go get` | 增删/升级模块依赖 | 改 go.mod 依赖版本；装二进制已改用 `go install` |
...
![工具链全景](./images/ch02-toolchain-overview.svg)
...
![标准 Go 项目结构](./images/ch02-project-structure.svg)
...
![CI/CD 流水线](./images/ch02-cicd-pipeline.svg)
...
# 3. 跳过校验和数据库（GOPRIVATE 已隐含跳过；
#    若只想跳过校验、仍希望走代理，可单独设 GONOSUMDB）
...
本章解决了一个核心问题：**如何为用户管理服务搭建生产级 Go 开发环境并配齐工具链**。
...
下一章我们先不急着写 HTTP API，而是用一个日志分析工具把 Go 的核心语法跑通。这个工具后续也会成为排查用户服务错误日志的基础能力。
