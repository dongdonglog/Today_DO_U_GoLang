...
```go
// 解析 multipart 表单的内存缓冲阈值（超出写临时文件），不是文件大小上限
r.MaxMultipartMemory = 8 << 20 // 8 MB

// 真正的大小限制：检查 file.Size
if file.Size > 10<<20 { // 10 MB
    c.JSON(400, gin.H{"error": "file too large"})
    return
}

// 更稳妥：用 http.MaxBytesReader 从连接层直接截断超大请求体，
// 避免恶意大请求先被完整读入
...
func (s *LocalStorage) Save(file *multipart.FileHeader) (string, error) {
    // 生成唯一文件名（相对路径，如 2025/01/uuid.jpg）
    filename := s.generateFilename(file.Filename)

    // 目标绝对路径 = basePath + 相对路径
...
func generateThumbnail(src string, dst string, width, height uint) error {
    ...
    // 生成缩略图：imaging.Thumbnail 保持比例缩放到指定尺寸内
...
```go
// 存储的文件名形如 2024/01/15/abc-123.jpg，含 /，
// 所以用 gin 的 *filepath 通配符（会带前导 /，需 TrimPrefix）
// r.GET("/api/v1/files/*filepath", downloadHandler)
...
func mergeChunksHandler(c *gin.Context) {
    ...
    // 先合并到临时文件，校验通过后再"提升"到正式目录，
    // 避免损坏/被篡改的文件在校验前就落到对外可访问的目录
    tmpPath := filepath.Join(tempDir, uploadID, "merged.tmp")
    if err := mergeChunks(uploadID, info.ChunkCount, tmpPath); err != nil {
        response.InternalError(c, "merge failed")
        return
    }

    // 在临时文件上做完整性校验
    if sha256File(tmpPath) != info.Sha256 {
        os.Remove(tmpPath)
    ...
    // 校验通过，原子性移动到正式目录
    finalPath, err := safePath(uploadDir, info.Filename)
    if err != nil {
        response.BadRequest(c, "invalid filename")
        return
    }
    if err := os.MkdirAll(filepath.Dir(finalPath), 0o755); err != nil {
        response.InternalError(c, "mkdir failed")
        return
    }
    if err := os.Rename(tmpPath, finalPath); err != nil {
        response.InternalError(c, "promote failed")
        return
    }
...
> 本章基于 **Go 1.25**、Gin v1.12.0、disintegration/imaging v1.6.2（图片缩略图）。API 与默认行为随版本变化，以对应版本官方文档为准。

- multipart/form-data RFC 7578：https://www.rfc-editor.org/rfc/rfc7578
